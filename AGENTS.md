...
- `CoverArtCard` — server component; the whole floating "card" from the moodboard (cover art, title, embedded single, countdown + platform list, microcopy). Content order is identity → when → how-to-not-miss-it → sound (cover art/title, then countdown + platforms, then the embedded single last). The working-backwards FAQ's original hypothesis was sound *before* when/how (a stranger needs "proof-of-real" before a countdown or button means anything) — a quick user test with a friend showed that mattered less in practice than getting to the countdown/CTA sooner, so the embed moved to last. Real usage beat the hypothesis; this is the version to keep unless testing says otherwise again.
...
- `PlatformSaveList` — the stacked row list from the moodboard; each row is a `PlatformSaveRow` (icon, label, button/"Coming soon" state — the latter is a plain non-focusable `<span>`, not a fake disabled button). A row's CTA is a `<button>` opening `PreSaveModal` when its `PlatformLink` has an `embedUrl` and the EP isn't released yet; otherwise it's a plain `<a>` (or "Coming soon" if `url` is null too).
- `PreSaveModal` — in-page modal (built on native `<dialog>` + `showModal()`, not a hand-rolled overlay) that hosts a platform's embeddable pre-save widget (currently CD Baby's Show.co widget for Spotify) so the OAuth hand-off to the streaming service never has to leave the site. Gets focus-trap/Escape-to-close for free from `<dialog>`; adds a ref-counted body-scroll lock on top (native `<dialog>` doesn't stop background scroll on its own) and a glassmorphism `::backdrop` blur. Always includes an "Open it directly" plain-link fallback to the widget's own hosted page, in case the embed doesn't load.
...
- `PreSaveModal` uses native `<dialog>`/`showModal()` for a real focus trap and Escape-to-close rather than a hand-rolled overlay; body scroll is explicitly locked while it's open (native `<dialog>` doesn't do this on its own) and released however the dialog closes.
...
- **Spotify EP pre-save: resolved via CD Baby's Show.co.** Earlier notes here claimed Spotify renders its own native pre-save prompt on `open.spotify.com/album/<uri>` for a scheduled-but-undelivered release — **that was wrong.** Visiting that URL pre-release shows an error/not-live page instead (caught by the artist testing the live link). There is no native Spotify pre-save; a real one requires either (a) a Spotify OAuth app storing each fan's refresh token and calling `PUT /me/library` at release time, or (b) a distributor-run tool that already has that OAuth infra approved. (a) was investigated and rejected: as of Feb 2026 Spotify Web API's Development Mode caps apps at **5 authorized users**, and lifting that (Extended Quota Mode) requires being an established, registered business with 250k+ MAU — completely out of reach for an indie debut EP, and building the OAuth callback + token DB + release-day cron job would itself have meant a backend + account system, violating this repo's own "no account system" non-negotiable. Went with (b) instead: CD Baby (this EP's distributor) offers Spotify pre-save for free via their built-in Show.co tool, set up from the CD Baby dashboard → Marketing Tools with Show.co → New Campaign → Spotify Pre-Save, identified by URI `spotify:album:0oDjObLTKHWZuBRAnQ2enA`, with gating/socials left off (the free tier doesn't support gating anyway). The published campaign gives two things, both wired into `lib/release.ts`'s Spotify entry: a plain minisite `url` (`show.co/...`, used as the fallback/"open directly" link) and an `embedUrl` (Show.co's own `<iframe>` widget), which `PreSaveModal` shows in an in-page modal so a fan never has to leave the site for the OAuth hand-off.
